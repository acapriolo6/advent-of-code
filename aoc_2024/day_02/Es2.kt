package aoc_2024.day_02

import common.readLines
import kotlin.math.abs

fun main() {
    val rawInput = readLines("${Es02::class.java.packageName.replace(".", "/")}/input_01.txt", true)
    val lines = createInputLists(rawInput)
    val result = Es02().calculateResult(lines)
    println(result)
}
class Es02 {
    fun calculateResult(lines: List<List<Int>>): Int {
        var count = 0
        for (line in lines) {
            count += if (isSafe(line, enableTolerance = true)) 1 else 0
        }
        return count
    }
    private fun isSafe(array: List<Int>, enableTolerance: Boolean = false): Boolean {
        val isAscending = array[0] < array[1]

        for (index in 1..array.lastIndex) {
            val distance = array[index - 1] - array[index]
            val absDistance = abs(distance)

            if (enableTolerance) {
                for (i in array.indices) {
                    if (isSafe(array.subList(0, i) + array.subList(i+1, array.size))) {
                        return true
                    }
                }
                return false
            } else {
                if (absDistance !in 1..3 || (isAscending && distance > 0) || (!isAscending && distance < 0)) {
                    return false
                }
            }
        }
        return true
    }
}
